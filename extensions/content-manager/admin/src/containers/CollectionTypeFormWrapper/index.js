...
const CollectionTypeFormWrapper = ({
  ...
  async function triggerWebhook(data) {
    ...
    try {
      const resp = await request(
        "https://7c6ef869a3ea6ba769c302e6f506ad14.m.pipedream.net",
        options
      );
      return resp;
    } catch (error) {
      console.log("Error trigerring webhook");
  ...
  const onPublish = useCallback(async () => {
    try {
      ...
      //trigger webhhook
      console.log("trigerring webhook....");
      //const resp = await triggerWebhook(data);
      // console.log({ resp });
  ...
  const onPreview = () => console.log("Sending webhook to preview server");
