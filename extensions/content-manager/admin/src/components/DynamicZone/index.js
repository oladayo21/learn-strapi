...
const DynamicZone = ({
  ...
  const getLength = () => dynamicDisplayedComponents.length;
  const [isOpen, setIsOpen] = useState(false);
  useEffect(() => {
    setIsOpen(getLength() === 0);
    ...
    <DynamicZoneWrapper>
      ...
      <ComponentWrapper>
        {dynamicDisplayedComponents.map((componentUid, index) => {
          ...
          const showUpIcon =
            isFieldAllowed && dynamicDisplayedComponentsLength > 0 && index > 0;
        ...
        <Wrapper>
          ...
          {hasRequiredError && !isOpen && !hasMaxError && (
            <div className="error-label">
              <FormattedMessage
                id={`${pluginId}.components.DynamicZone.required`}
              />
            </div>
          )}
          {hasMaxError && !isOpen && (
